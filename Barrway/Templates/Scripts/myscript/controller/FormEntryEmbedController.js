(function () {
    'use strict';
    FormGeneratorApp.controller('FormEntryEmbedController', function ($scope, $rootScope, CookiesPersistenceService, $http, $state, $location, $window, $ngBootbox, $timeout, mainService, notifierService, $stateParams, DataService) {
        ...
        function pickDefaultValues($this) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result) {
                                ...
                                if (dependent_fields_in_table_arr != undefined) {
                                    var resultTemp = {};
                                    if (result.length > 0) {
                                        resultTemp = result[0];
                                    }
        ...
        $scope.getFormSettings = function (param) {
            ...
            mainService.manageForm("ManageForm", param)
                .then(function (response) {
                    if (response.data != null && angular.isDefined(response.data)) {
                        //console.log(response.data,'check PP controls,status and its amount')
                        //console.log(response.data[0].fields, 'page control load')
            ...
            _.each(compareList, function (item) {
                ...
                if (!DataService.isEmpty(exists)) {
                    ...
                    if (!DataService.isEmpty(temp[0].name)) {
                        ...
                        result += item.label + " Should be compare to " + exists.label + " ";
        ...
        function modifyFormData() {
            ...
            $('#customFormNew').find($('input[type="checkbox"]:not(:checked)')).each(function () {
                if ($.inArray(this.name, names) === -1) {
                    temp.push({ name: this.name, value: 'false' });
                }
            });
        ...
        function GeneratedFormData(dataParam) {
            $rootScope.$emit("ShowLoading");
            console.log(dataParam, 'paramsss')
            mainService.manageGeneratedFormData("GeneratedFormData", dataParam)
                .then(function (response) {
                    if (response.data != null && angular.isDefined(response.data)) {
                        if (!DataService.isEmpty(response.data.Message)) {
