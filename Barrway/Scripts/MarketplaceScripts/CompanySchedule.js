...
var agendaGroupEvents = [];
$(document).ready(async function () {

    /*$(".calendar-selector").change(function () { $('.calendar-selector').val($(this).val()); });*/
    ...
    if (calenderSettings.length > 0) {
        ...
        //calendar service bind
        getServiceMaster(activityConfig);
        //calendar location bind
        getLocationMaster(calenderSettings);
        //calendar service provider bind
        getServiceProviderData(calenderSettings);
        //calendar bind change event bind
        calendarDropDwnEvent();
...
function marcketplaceCalendar(calenderType, calenderData, resourceData, resColumns, activityFormData, activityColumn, activityEvents, allowTabs) {
    ...
    var defaultOptions = {
        ...
        loading: function (bool) {
            ...
            if ($('#timeline-resource-view div.calendar .fc-toolbar').length > 0) {
        ...
        eventRender: function (event, element) {
            ...
            if (!checkShowTitle()) {
                ...
                if (current_tab == "agenda-view") {
                    if (window.innerWidth <= 576) {
                        if (!agendaGroupEvents.find(x => moment(x.start.format()).format("YYYY-MM-DD") == moment(event.start.format()).format("YYYY-MM-DD"))) {
                            agendaGroupEvents.push(event);
                            element.addClass("agenda-min-e");
                            element.find(".fc-content").remove();
                            element.append("<div class='fc-content'></div><div class='fc-content'>•</div>");
                        } else {
                            element.addClass("agenda-min-e");
                            element.find(".fc-content").remove();
                        }
                ...
                element.css({
                    background: current_tab == "agenda-view" && window.innerWidth <= 576 ?"none!important":(serviceColor && serviceColor != "" ? serviceColor : "rgb(255, 255, 255)"),
                    /*borderColor: "#aaa",*/
                    padding: 0,
                    border: "none",
                    borderRadius: 5,
        ...
        eventClick: async function (calEvent, jsEvent, view) {
            ...
            if (window.innerWidth <= 576) {
                var current_tab = $('#tabs .ui-tabs-panel:eq(' + $("#tabs").tabs("option", "active") + ')').attr('id');
                if (current_tab == "agenda-view") {
                    tabsActive($('a[href="#list-view"]').parent().index());
    ...
    myOptions = {
        ...
        select: function (start, end) {
            //agenda view click open list view day
            var current_tab = $('#tabs .ui-tabs-panel:eq(' + $("#tabs").tabs("option", "active") + ')').attr('id');
            if (current_tab == "agenda-view") {
                tabsActive($('a[href="#list-view"]').parent().index());
...
function switchToDropdown() {
    let listoptions = (is5CType) ? [{ label: 'Year', value: 'listYear' }] : [{ label: 'Day', value: 'listDay' }, { label: 'Week', value: 'listWeek' }, { label: 'Month', value: 'listMonth' }, { label: 'Year', value: 'listYear' }];

    var calendars = [{ name: "timeline-resource-view", options: [{ label: 'Day', value: 'timelineDay' }, { label: 'Week', value: 'timelineWeek' }, { label: 'Month', value: 'timelineMonth' }, { label: 'Year', value: 'timelineYear' }], select_id: "select-timeline", style: "width: auto;display: inline;" },
        { name: "list-view", options: listoptions, select_id: "select-list-view", style:"width:auto;display: inline;" },
        { name: "vertical-resource-view", options: [{ label: 'Day', value: 'agendaDay' }, { label: '2 Days', value: 'agendaTwoDays' }, { label: '3 Days', value: 'agendaThreeDays' }, { label: 'Week', value: 'agendaWeek' }], select_id: "select-vertical-resource-view", style:"width: auto;display: inline;" }];

    calendars.forEach(calendar => {
        let name = calendar.name;
        let select_name = calendar.select_id;
        $('#'+name+' div.calendar .fc-right').hide();
        // Create a dropdown
        if ($('#' + select_name).length == 0) {
            ...
            calendar.options.forEach(x => {
                innerHtml += ` <option value="${x.value}">${x.label}</option>`;
            });
            ...
            var header = document.querySelector('#' + name + ' div.calendar .fc-toolbar .fc-left');
            if ($('#' + name + ' div.calendar .fc-toolbar .fc-left').length > 0) {
                $('#' + name + ' div.calendar .fc-toolbar .fc-left').after(select);
...
function switchToButtons() {
    ...
    var calendars = [{ name: "timeline-resource-view", select_id: "select-timeline" },
    { name: "list-view", select_id: "select-list-view" },
        { name: "vertical-resource-view", select_id: "select-vertical-resource-view" }];
    // Remove the dropdown
    ...
    calendars.forEach(calendar => {
        ...
        let name = calendar.name;
        let select_name = calendar.select_id;
        var select = document.querySelector('#' + select_name);
        if (select) {
            select.parentNode.removeChild(select);
        }

        // Restore default buttons
        $('#'+name+' div.calendar .fc-right').show();
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                wrapper.innerHTML = `<div>
                                    ...
                                    ${((eventData.NEED_ONLINE_PAYMENT == 'N') ? `<div>You will need to handle the payment with the company's staff.</div>` : "")}<br />
                                ...
                                if (response.Data == 0 || eventData.NEED_ONLINE_PAYMENT == 'N') {
                                    buttonsWrap = {
                                        cancel: "Cancel",
                                        confirm: "Yes"
                                    }
