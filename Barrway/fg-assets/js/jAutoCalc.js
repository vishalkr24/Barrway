    ...
    $.fn.jAutoCalc = function (method) {
        ...
        var getFieldSelector = function (field) {
            if (/^[a-zA-Z].*/.test(field)) {
                var targetField = ':input[name="' + field + '"]';
                if (!$(targetField).length)
                    targetField = getFieldSelectorByClass(field);
                return targetField;
            }
            return field;
        };
        var getFieldSelectorByClass = function (field) {
            if (/^[a-zA-Z].*/.test(field)) {
                return 'input.' + field;
            }
            return field;
        ...
        var doCalc = function (eq, fields, result, ctx) {
            ...
            for (i = 0; i < fields.length; i++) {
                ...
                if(fieldValue.match(regEx)){
                    var dateString = new Date(fieldValue);
                    fieldValue = dateString.getTime();
                }
