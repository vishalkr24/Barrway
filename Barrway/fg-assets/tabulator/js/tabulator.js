...
;(function (global, factory) {
	...
	Filter.prototype.initializeColumn = function (column, value) {
        ...
        if (pathArray.length) {
            ...
            tabulatorFilter = "tabulator-" + self.table.options.persistenceID + "-filter";
            //console.log(tabulatorFilter)
            if (tabulatorFilter !== '') {
                    ...
                    typingTimer = setTimeout(function () {             
                        ...
                        if (tabulatorFilter !== '') {
                            ...
                            if (filterArr.length == 0) {
                                filterArr = temp;
                                filterArr['name'] = "applications";                             
                                filterArr['filters'] = {};
                                filterArr['filters'][column.definition.field] = editorElement.value;
                                filterArr = [filterArr];
                            }
                            else {
                                filterArr[0]['name'] = "applications";   
                                filterArr[0]['filters'][column.definition.field] = editorElement.value;
                            }                       
                            self.table.modules.persistence.writers.local("tabulator-" + self.table.options.persistenceID, "filter", filterArr)
                            self.table.modules.filter.addFilter(column.definition.field, column.definition.type, editorElement.value, filterArr);
                        }
	...
	Filter.prototype.clearHeaderFilter = function () {
        ...
        console.log("tabulator - E - filter"+localStorage.getItem("tabulator-E-filter"))
		this.headerFilters = {};
	...
	Persistence.prototype.initialize = function () {
        ...
        if (this.config.filter) {
            retreivedData = this.retreiveData("filter");

            if (retreivedData) {
                if (retreivedData.length > 0)
                    var filData = retreivedData[0].filters;
                this.table.options.initialHeaderFilter = [];
                for (var dataFill in filData) {
                    this.table.modules.filter.headerFilters[dataFill] = { value: filData[dataFill], type: "like", params: {} }; 
                    this.table.options.initialHeaderFilter.push({ field: dataFill, value: filData[dataFill] });
                }             
	...
	Persistence.prototype.save = function (type) {
        ...
        if (this.writeFunc) {
            if (this.config.filter && type=="filter") {
                if (localStorage.getItem(this.id + "-" + type) != null && localStorage.getItem(this.id + "-" + type).length) {
